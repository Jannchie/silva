...
队列 = "接下来标什么"，由 silva 侧算法生成、pictoria 侧消费：
...
- silva 侧脚本生成队列 CSV / JSON，通过 pictoria API 或 CLI 导入。
- 队列策略由 silva 演进：冷启动（coverage kernel 多样性采样 + 旧总分分层）→
  OOF 分歧采样 → head 边界带 pair 采样。pictoria 不需要懂策略，只按队列出题。
...
**queue 生成脚本**（silva `scripts/`）：

- `make_queue_coldstart.py`：coverage kernel 多样性 + 旧总分分层，从 train split 抽。
- `make_queue_oof.py`：维度版 oof_audit（既有工具改造）。
- `make_queue_pairs.py`:head 边界带（latent 接近的样本对）出 pair。
