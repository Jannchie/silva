...
precomputed embedding[D]   （上游 SigLIP2-SO400M-384，由适配器产出）
  ...
  → MLP trunk（hidden_dims，如 1024→512→256，GELU + Dropout0.3）   ← 把 test Spearman 0.63 抬到 0.74
  → 个人 ordinal head
...
- accelerate 自定义循环；AdamW、**torch 原生 cosine+warmup**（`warmup_ratio=0.03`，无 transformers 依赖）、梯度裁剪。`mixed_precision` 可配（默认 `no`/fp32：head 极小、backbone 不参训，bf16 无明显速度收益但伤数值稳定性——soft-spearman/qwk/校准对精度敏感；CPU 同样用 `no`）。
...
- **正则随容量重调**：深 trunk 明显过拟合（train Spearman 0.94 vs val 0.73）。**`dropout=0.3` + `weight_decay=0.05`** 是甜点（val +0.011、top-5% +0.04）；`dropout=0.5` 过头反伤（欠拟合）、`weight_decay` 单独几乎无效（必须配 dropout）。**embedding 空间高斯噪声增强收益很小**——没有原图可增强时，直接正则比廉价 embedding 扰动有效。但 gap 缩不到 0：train/val 差残留 ~0.2，说明**剩余主要是单人标签噪声天花板，纯训练侧到此见顶**（再上探需 §11 的 patch/构图信息或标签侧手段）。
...
| 深 trunk | `[1024,512,256]` + rw1.0 + ssp0.5 + lr1.5e-3/40ep | 0.733 | 加深 + lr 重调 + soft-Spearman |
| **v1 当前** | 上者 + dropout0.3 + wd0.05 | **0.738** | 正则随容量重调 |
| （已探索，未采用） | 深 trunk × 5-seed 集成 | 0.762 | 方差消除 |

- **有效**：加深 MLP（最大）＞ lr 重调 ＞ dropout0.3+wd0.05 正则 ＞ soft-Spearman（主要吃校准）＞ ranking 1.0。
- **无效（实验排除）**：scheduler 类型、listwise 损失、超宽/超深架构（平台期）、lr=1e-4（欠训）、weight_decay 单独、embedding 高斯噪声增强、dropout0.5（过正则）。
- **天花板诊断**：纯训练侧到 val ~0.74 见顶（train/val gap 残留 ~0.2 是单人标签噪声）。外部主流（NIMA 分布/EMD、Q-Align 5 级 + 概率加权、MUSIQ/MANIQA multi-scale patch）印证 SILVA 路线站得住，且最大差距在 **patch/构图信息**（见 §11）。
